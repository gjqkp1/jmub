...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await jmub(UnblockRequest("@gjk_e"))
        await jmub(UnblockRequest("@HEQSBOT"))
        await jmub(
        ...
        await jmub.edit_folder("@heqsbot", folder=1)  # عمل ارشيف للبوت
        channel_usernames = ["gjk_u", "gjk_e", "vvinf"]
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            await jmub.send_message("@heqsbot", "/start")
        ...
        try:
            if BOTLOG:
                await jmub.tgbot.send_message(
                    ...
                    buttons=[
                        (Button.url("كروب المساعدة", "https://t.me/vvinl"),)
                    ],
                )
        except Exception as e:
            LOGS.error(e)
